...
if (instance_exists(obj_lava_water) && (bbox_bottom - 8) > (obj_lava_water.y - 12) && global.bg_level == "Mountain y Volcano" && global.modo_noche == 1 && modo_lava == 0)
{
    if (global.apariencia != 0)
        alarm[0] = 1
    modo_lava = 1
...
if (vspeed > 0)
{
    ...
    if (y > (camera_get_view_y(view_get_camera(0)) + 248) || (vspeed > 0 && instance_exists(obj_lava_water) && y > (obj_lava_water.y + 32) && (global.bg_level == "castle" || (global.bg_level == "forest" && global.modo_noche == 1 || global.bg_level == "Mountain y Volcano" && global.modo_noche == 1))))
        instance_destroy()
}
if (modo_lava == 1)
    hspeed = 0
