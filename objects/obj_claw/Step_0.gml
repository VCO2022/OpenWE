...
repeat snelheiditeraties
{
    ...
    if (i < segmenten)
    {
        var xx = (knopen_x[(i + 1)] - knopen_x[i])
        var yy = (knopen_y[(i + 1)] - knopen_y[i])
        var d = sqrt(((xx * xx) + (yy * yy)))
        if (d > 0.001)
        {
            xx /= d
            yy /= d
...
if (hold == 2 && instance_exists(obj_mario))
{
    ...
    if collision_rectangle(obj_mario.bbox_right, (obj_mario.bbox_top + 4), (obj_mario.bbox_right + 1), (obj_mario.bbox_bottom - 1), obj_spring_land, 1, 0)
        ...
        knopen_xspeed[segmenten] = 2
    var col_one_right = collision_rectangle((obj_mario.bbox_left - 1), (obj_mario.bbox_top + 4), obj_mario.bbox_left, (bbox_bottom - 1), obj_onewaygate_right, 1, 0)
    var col_one_left = collision_rectangle(obj_mario.bbox_right, (obj_mario.bbox_top + 4), (obj_mario.bbox_right + 1), (obj_mario.bbox_bottom - 1), obj_onewaygate_left, 1, 0)
    if col_one_right
    {
