    ...
    if (instance_exists(obj_invincibility) && global.effect_star == 1)
    {
        ...
        if (global.apariencia == 0 && obj_mario.sprite_index == scr_marioclimb())
        {
            if (global.powerup == -77)
                ...
                mpos = 6
            draw_sprite_ext(sprite_index, image_index, (obj_mario.x - (mpos * direct)), ((obj_mario.y + pos_y) + 1), direct, 1, rotacion, c_white, obj_mario.image_alpha)
        }
        else if instance_exists(obj_mario_pballon)
            draw_sprite_ext(sprite_index, image_index, (obj_mario_pballon.x + obj_mario_pballon.xDiff_h), (obj_mario_pballon.y + obj_mario_pballon.yDiff_h), 1, 1, obj_mario_pballon.direction, c_white, obj_mario.image_alpha)
        else
            draw_sprite_ext(sprite_index, image_index, obj_mario.x, ((obj_mario.y + pos_y) + 1), direct, 1, rotacion, c_white, obj_mario.image_alpha)
        shader_reset()
    }
}
