...
function index_powerups() {
	...
	var powerup_count = 16;

	//Loop through and get every asset
	for (var i = 0; i < powerup_count; i++) {

		global.attack_sprite[i] =		asset_get_index("spr_" + string(style_get_name(i)) + + string(style_get_name(i)) + string(powerup_get_name(i)) + "_attack");
		...
		global.idle_sprite[i] =			asset_get_index("spr_" + string(style_get_name(i)) + + string(character_get_name(i)) + string(powerup_get_name(i)) + "_idle");
